    ...
    if (window.requestAnimationFrame && document.documentElement.classList) {
        ...
        var toggleElement = function () {
            if (revealer.length == 0) {
                revealer = document.querySelectorAll('.revealer');
            }
            for (var i = 0; i < revealer.length; i++) {

                if (checkVisibility(revealer[i]) && !(revealer[i].classList.contains('shown'))) {

                    //revealer[i].style.visibility = 'visible';

                    var gauge = Gauge(
                        document.getElementById(revealer[i].id), {
                            max: 100,
                            value: 10
                        }
                    );

                    var rate = revealer[i].firstChild.id;

                    (function loop(Gauge, Rate) {
                        gauge.setValueAnimated(rate, 1);
                        window.setTimeout(loop, 6000);
                    })();

                    revealer[i].classList.add('shown');
